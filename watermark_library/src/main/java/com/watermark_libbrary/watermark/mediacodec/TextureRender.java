package com.watermark_libbrary.watermark.mediacodec;
...
import com.watermark_libbrary.watermark.filter.AFilter;
import com.watermark_libbrary.watermark.filter.GPUImageFilter;
import com.watermark_libbrary.watermark.filter.GroupFilter;
import com.watermark_libbrary.watermark.filter.NoFilter;
import com.watermark_libbrary.watermark.filter.RotationOESFilter;
import com.watermark_libbrary.watermark.filter.WaterMarkFilter;
import com.watermark_libbrary.watermark.utils.EasyGlUtils;
import com.watermark_libbrary.watermark.utils.MatrixUtils;
...
//        mShow.setMatrix(MatrixUtils.flip(MatrixUtils.getOriginalMatrix(), false, true));
        ...
        if (info.watermark != null) {
            //默认加上水印 可以取消掉
            Bitmap bitmap = null;
            if (info.watermark.getBitmapWatermark() != null) {
                bitmap = info.watermark.getBitmapWatermark();
            }
            if (bitmap != null) {
                WaterMarkFilter waterMarkFilter = new WaterMarkFilter(resources);
                waterMarkFilter.setWaterMark(bitmap);
                int leftMargin = (int) (20 * info.width / 1080f);
                int topMargin = (int) (20 * info.width / 1080f);
                waterMarkFilter.setPosition(info.width - bitmap.getWidth() - leftMargin, topMargin /*bitmap.getHeight()*/, info.watermark.getWidth(), info.watermark.getHeight());
